...
console.log('黑黑犊雉白 →', xiao.name, xiao.combo, xiao.points + '筴');

const bald = Chupu.resolveCai(['bai', 'xuan', 'xuan', 'du', 'bai']);
if (bald.combo !== '黑黑犊白白' || bald.name !== '秃' || bald.points !== 4) {
  throw new Error('凡白+双雉黑+犊+犊白 应判秃4筴');
}
console.log('黑黑犊白白 →', bald.name, bald.combo, bald.points + '筴');

const lu = Chupu.resolveCai(['xuan', 'xuan', 'xuan', 'du', 'du']);
if (lu.name !== '卢' || lu.combo !== '黑黑黑犊犊') throw new Error('卢采组错误');
console.log('卢 →', lu.name, lu.combo, lu.points + '筴');
